...
app.get("/filter", (req, res) => {
  ...
  const typeJoke = jokes.filter((joke) => joke.jokeType === type);
  res.json(typeJoke);
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const findIndex = jokes.findIndex((joke) => joke.id === id);
  if (findIndex !== -1) {
  const exist = jokes[findIndex];
  const replace = {
    id: id,
    jokeText: req.body.text || exist.jokeText,
    jokeType: req.body.type || exist.jokeType,
  }
  jokes[findIndex] = replace;
  res.json(replace);
} else {
...
app.delete("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const indexnum = jokes.findIndex((joke) => joke.id === id);
  if (indexnum > -1) {
    jokes.splice(indexnum, 1);
    res.status(200);
    res.json(`array index ${indexnum} has been deleted`);
  } else {
